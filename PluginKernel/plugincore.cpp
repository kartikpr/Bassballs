...
PluginCore::PluginCore()
{
	...
	MoogLPFParameters filterOneParams = filterOne.getParameters();
	filterOneParams.fc = fc1;
	filterOneParams.Q = q1;
	filterOne.setParameters(filterOneParams);

	MoogLPFParameters filterTwoParams = filterOne.getParameters();
	filterTwoParams.fc = fc2;
	filterTwoParams.Q = q2;
	filterTwo.setParameters(filterTwoParams);

	//filterOneParams.bassBoost_Pct = bassBoost_Pct;
	//filterOneParams.enableAutoLimiter = (enableAutoLimiter == 1);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
    ...
    {
		// --- pass through code: change this with your signal processing
		double xnL = inputGain * processFrameInfo.audioInputFrame[0];
		double xnR = inputGain * processFrameInfo.audioInputFrame[1];
	


		// --- calc threshold
		double threshValue = pow(10.0, threshold / 20.0);
		// --- detect the signal
		double detect_dB_L = detector.processAudioSample(xnL);
		double detect_dB_R = detector.processAudioSample(xnR);
		double detectValue_L = pow(10.0, detect_dB_L / 20.0);
		double detectValue_R = pow(10.0, detect_dB_R / 20.0);
		double deltaValue = ((detectValue_L + detectValue_R) / 2) - threshValue;

		MoogLPFParameters filterOneParams = filterOne.getParameters();
		MoogLPFParameters filterTwoParams = filterTwo.getParameters();
		//filterParams.fc = fc;
		// 
		...
		if (deltaValue > 0.0)
		{
			...
			// --- best results-linear when detector is in dB mode
			modulatorValue = (deltaValue * (sensitivity/0.75));
			// --- calculate modulated frequency
			filterOneParams.fc =
				doUnipolarModulationFromMin(modulatorValue,
					fc1, kMaxFilterFrequency);
			filterTwoParams.fc =
				doUnipolarModulationFromMin(modulatorValue,
					fc1, kMaxFilterFrequency) * fc2;
		}
		...
		double filterOneOutput_L =  filterOne.processAudioSample(xnL);
		double filterOneOutput_R =  filterOne.processAudioSample(xnR);

		double filterTwoOutput_L =  filterTwo.processAudioSample(xnL);
		double filterTwoOutput_R =  filterTwo.processAudioSample(xnR);

		// --- perform the filtering operation
		processFrameInfo.audioOutputFrame[0] = (filterOneOutput_L + filterTwoOutput_L);
		processFrameInfo.audioOutputFrame[1] = (filterOneOutput_R + filterTwoOutput_R);
...
bool PluginCore::initPluginParameters()
{
	...
	// --- continuous control: fc1
	piParam = new PluginParameter(controlID::fc1, "fc1", "Hz", controlVariableType::kDouble, 20.000000, 10000.000000, 1000.000000, taper::kVoltOctaveTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&fc1, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Q1
	piParam = new PluginParameter(controlID::q1, "Q1", "Units", controlVariableType::kFloat, 2.000000, 20.000000, 2.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&q1, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: Attack
	piParam = new PluginParameter(controlID::attack, "Attack", "mSec", controlVariableType::kDouble, 1.000000, 250.000000, 20.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&attack, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Release
	piParam = new PluginParameter(controlID::release, "Release", "Units", controlVariableType::kDouble, 1.000000, 2000.000000, 500.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&release, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Threshold
	piParam = new PluginParameter(controlID::threshold, "Threshold", "dB", controlVariableType::kDouble, -20.000000, 0.000000, -6.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&threshold, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Sensitivity
	piParam = new PluginParameter(controlID::sensitivity, "Sensitivity", "Units", controlVariableType::kDouble, 0.250000, 0.900000, 0.900000, taper::kAntiLogTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&sensitivity, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: fc2 offset
	piParam = new PluginParameter(controlID::fc2, "fc2 offset", "Units", controlVariableType::kDouble, 2.000000, 5.000000, 2.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&fc2, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Q2
	piParam = new PluginParameter(controlID::q2, "Q2", "Units", controlVariableType::kDouble, 2.000000, 10.000000, 2.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&q2, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- discrete control: Distortion
	piParam = new PluginParameter(controlID::distortion, "Distortion", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&distortion, boundVariableType::kInt);
	...
	// --- continuous control: DC Offset
	piParam = new PluginParameter(controlID::dcOffset, "DC Offset", "Units", controlVariableType::kFloat, -1.000000, 0.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	...
	piParam = new PluginParameter(controlID::inputGain, "Input Gain", "Units", controlVariableType::kFloat, 1.000000, 8.000000, 1.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&inputGain, boundVariableType::kFloat);
	addPluginParameter(piParam);
	...
	setParamAuxAttribute(controlID::fc1, auxAttribute);

	// --- controlID::q1
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::q1, auxAttribute);
	...
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::threshold, auxAttribute);
	...
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::fc2, auxAttribute);

	// --- controlID::q2
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::q2, auxAttribute);

	// --- controlID::distortion
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741827);
	setParamAuxAttribute(controlID::distortion, auxAttribute);
	...
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::inputGain, auxAttribute);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::fc1, 1000.000000);
	setPresetParameter(preset->presetParameters, controlID::q1, 2.000000);
	setPresetParameter(preset->presetParameters, controlID::attack, 20.000000);
	setPresetParameter(preset->presetParameters, controlID::release, 500.000000);
	setPresetParameter(preset->presetParameters, controlID::threshold, -6.000000);
	setPresetParameter(preset->presetParameters, controlID::sensitivity, 0.900000);
	setPresetParameter(preset->presetParameters, controlID::fc2, 2.000000);
	setPresetParameter(preset->presetParameters, controlID::q2, 2.000000);
	setPresetParameter(preset->presetParameters, controlID::distortion, -0.000000);
	...
	setPresetParameter(preset->presetParameters, controlID::inputGain, 1.000000);
	addPreset(preset);

	// --- Preset: bass1
	preset = new PresetInfo(index++, "bass1");
	initPresetParameters(preset->presetParameters);
	setPresetParameter(preset->presetParameters, controlID::fc1, 159.883560);
	setPresetParameter(preset->presetParameters, controlID::q1, 7.220000);
	setPresetParameter(preset->presetParameters, controlID::attack, 43.654999);
	setPresetParameter(preset->presetParameters, controlID::release, 809.845032);
	setPresetParameter(preset->presetParameters, controlID::threshold, -13.099999);
	setPresetParameter(preset->presetParameters, controlID::sensitivity, 0.309731);
	setPresetParameter(preset->presetParameters, controlID::fc2, 3.785000);
	setPresetParameter(preset->presetParameters, controlID::q2, 5.200000);
	setPresetParameter(preset->presetParameters, controlID::distortion, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::drive, 1.000000);
	setPresetParameter(preset->presetParameters, controlID::dcOffset, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::inputGain, 3.300000);
	addPreset(preset);

	// --- Preset: bass 2
	preset = new PresetInfo(index++, "bass 2");
	initPresetParameters(preset->presetParameters);
	setPresetParameter(preset->presetParameters, controlID::fc1, 67.193794);
	setPresetParameter(preset->presetParameters, controlID::q1, 5.240000);
	setPresetParameter(preset->presetParameters, controlID::attack, 66.064987);
	setPresetParameter(preset->presetParameters, controlID::release, 519.989868);
	setPresetParameter(preset->presetParameters, controlID::threshold, -17.500000);
	setPresetParameter(preset->presetParameters, controlID::sensitivity, 0.349718);
	setPresetParameter(preset->presetParameters, controlID::fc2, 4.130000);
	setPresetParameter(preset->presetParameters, controlID::q2, 5.440000);
	setPresetParameter(preset->presetParameters, controlID::distortion, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::drive, 2.125000);
	setPresetParameter(preset->presetParameters, controlID::dcOffset, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::inputGain, 6.530001);
	addPreset(preset);

	// --- Preset: guitar
	preset = new PresetInfo(index++, "guitar");
	initPresetParameters(preset->presetParameters);
	setPresetParameter(preset->presetParameters, controlID::fc1, 107.090752);
	setPresetParameter(preset->presetParameters, controlID::q1, 7.850000);
	setPresetParameter(preset->presetParameters, controlID::attack, 193.054993);
	setPresetParameter(preset->presetParameters, controlID::release, 2000.000000);
	setPresetParameter(preset->presetParameters, controlID::threshold, -20.000000);
	setPresetParameter(preset->presetParameters, controlID::sensitivity, 0.286818);
	setPresetParameter(preset->presetParameters, controlID::fc2, 3.934999);
	setPresetParameter(preset->presetParameters, controlID::q2, 5.080000);
	setPresetParameter(preset->presetParameters, controlID::distortion, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::drive, 1.495000);
	setPresetParameter(preset->presetParameters, controlID::dcOffset, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::inputGain, 2.995000);
	addPreset(preset);
